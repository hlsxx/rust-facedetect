...
fn try_run() -> Result<()> {
    if !std::path::Path::new(CASCADE_XML_FILE).exists() {
        panic!("Cascade file not found at: {}", CASCADE_XML_FILE);
    }
...
fn detect_faces(classifier: &mut objdetect::CascadeClassifier, image: Mat) -> Result<Vector<Rect>> {
